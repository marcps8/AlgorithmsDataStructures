...
def augmenting(G: Graph, s: str, t: str) -> bool:
    ...
    Pre: G is a connected directed graph with a source s, a sink t, a
    nonnegative capacity c(u, v) on each edge, and a non-negative flow f(u, v) on each edge
    Post: return True if there is an augmenting path in G from s to t, False otherwise
    ...
    while len(open) > 0:
        ...
        nei = G.neighbors(n)
        for vecino in nei:
            # Invariant: nei is the list of neighbors, t is the target node, visited has the already visited nodes
            # Variant: vecino is the current neighbor, open has the nodes to be visited
            if G.capacity(n,vecino) - G.flow(n,vecino) > 0: #there's still capacity
                if vecino == t: return True
                else:
                    if vecino not in visited:
                        open.insert(0,vecino) #insert neighbor at the beggining of the open list to visit it the first
    return False
def augmenting_extended(G: Graph, s: str, t: str) \
    ...
    Pre: G is a connected directed graph with a source s, a sink t, a
    nonnegative capacity c(u, v) on each edge, and a non-negative flow f(u, v) on each edge
    Post: return True if there is an augmenting path in G from s to t and a List of tuples representing that path,
    ...
    while stack:
        # Invariant: G is the graph, s is the source node, t is the target node
        # Variant: stack has the nodes to be visited and their path from s, visited has the already visited nodes,
        #           nei is the list of neighbors of the node n, n is current node, path is a list of tuples containing the path from s to each node
        (n, path) = stack.pop()
        visited.append(n)
        nei = G.neighbors(n)
        for vecino in nei:
            # Invariant: G is the graph, t is the target node, n is current node, nei is the list of current neighbors of the node n,
            #            visited has the already visited nodes
            # Variant: vecino, stack has the nodes to be visited and their path from s,
            #           path is a list of tuples containing the path from s to each node
            if G.capacity(n, vecino) - G.flow(n, vecino) > 0:  # there's still capacity
                if vecino == t:
                    return True, path+[(n,vecino)]
                else:
                    if vecino not in visited:
                        stack.insert(0,(vecino,path+[(n,vecino)])) # insert neighbor at the beggining of the stack list to visit it the first
